  ...
  if (!!values && values.constructor === Array) {
    for (j = 0, len1 = values.length; j < len1; j++) {
      ...
      if (v === null || v === void 0 || t === 'number' || t === 'string'){
        ...
        params.push(v.valueOf());
      } else if (t !== 'function') {
        params.push(v.toString());
        console.warn('addStatement - parameter '+j+' of type <'+t+'> ('+v+') converted to string using toString()', sql );
      } else {
        let errorMsg = 'Unsupported parameter type <'+t+'> (parameter '+j+') found in addStatement('+sql+')';
